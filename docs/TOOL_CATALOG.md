...
The server currently registers 177 MCP tools.
...
Core Live control tools expose field-level schemas for agent planning and validation. For example, tempo uses `tempo`, track/scene creation uses `name` and optional indexes, clip tools use `track_index` and `clip_slot_index`, track and return-track mixer tools use bounded `value`, and device parameter writes use explicit track/device/parameter indexes. Real writes still require `dry_run=false` plus `ABLETON_MCP_ENABLE_WRITE=1`.
...
| Production readiness | `ableton_get_production_readiness`, `ableton_ui_control_consent_status`, `ableton_plan_ui_control_session` |
...
| Write-gated Live control | `ableton_set_tempo`, `ableton_create_clip`, `ableton_set_clip_gain`, `ableton_transpose_clip`, `ableton_set_return_track_volume`, `ableton_set_device_parameter` |
