...
- Compact large-set reads through `ableton_list_tracks_compact`, `ableton_get_track_detail`, and `ableton_get_clip_detail`. Use these before full snapshots on dense projects because Max for Live can truncate very large JSON responses.
...
- Targeted read tools accept typed `track_index`/`device_index` aliases in addition to legacy `track_id`/`device_id` selectors. The Max bridge uses the typed index for track mixer, clip-slot, device-list, arm/mute/solo, and track-rename actions, so generated runbook calls can inspect or mutate the intended track instead of falling back to the selected track.
- `ableton_browse_live_devices` remains offline by default, but with `check_bridge=true` it uses the `browser_device_tree` bridge action to read bounded `live_app browser` category and BrowserItem metadata for instruments, audio effects, MIDI effects, Max for Live, and plugins. `max_items` is a total per-category item budget, with `max_depth` capped at 2. This is discovery only; it does not load, hot-swap, click, type, or insert devices.
- Read-only live automation target mapping through `ableton_extract_automation_summary`, including track volume, pan, sends, and bounded device parameters with dry-run current-value write templates.
- Audio clip creation from an approved local sample path with `ableton_load_preset_or_sample` in `audio_clip` mode.
- MIDI note insertion and deterministic bounded humanization when the loaded Ableton/Max LiveAPI context supports the modern note API. `ableton_insert_midi_notes replace_existing=true` and `ableton_humanize_midi_clip` both read the bounded note range with `get_notes_extended`, remove that range with `remove_notes_extended`, and only then call `add_new_notes`; if the read/remove methods are unavailable they return `unsupported: true` before adding notes.

The bridge returns explicit `unsupported: true` results for operations that are not reliable in the current LiveAPI context, including generic device/preset insertion, macro mapping, groove application, Arrangement View clip placement/editing, automation breakpoint writes, Ableton export/save-as, and clip quantization paths that cannot be proven for the selected target. MCP dry-runs for those tools report the limitation up front with next-step hints instead of pretending execution is available. MCP clients should treat those responses as setup or capability limits, not as success.
