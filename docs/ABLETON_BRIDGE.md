...
- Track mixer reads now include send parameters with `send_index`, value, and matching return-track names where LiveAPI exposes them. `ableton_set_track_send` validates the requested send index against the current track before writing, so agents should inspect `ableton_get_track_mixer` and `ableton_list_return_tracks` before routing layers to reverb, delay, or texture returns.
