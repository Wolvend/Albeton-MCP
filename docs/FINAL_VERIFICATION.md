...
For this verification run, the host HTTP service was restarted after the latest build so Docker could query the rebuilt 214-tool server. A direct Streamable HTTP MCP probe reported `toolCount: 214`, `hasLaunchAudit: true`, `auditMode: "ready_for_offline_planning"`, `okForDefaultClientUse: true`, `bridgeReachable: false`, and `safeToolCount: 138`. The rebuilt HTTP surface also exposes the user-gated concept device UI session only outside the safe client allowlist; real UI/mouse work still requires `ABLETON_MCP_ENABLE_UI_CONTROL=1`.
