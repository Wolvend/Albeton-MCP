...
Result: succeeded. Doctor reported 8 checks, 0 failures, and 2 runtime warnings because the UI driver on `127.0.0.1:17365` and Max for Live bridge on `127.0.0.1:17364` were not loaded. The HTTP transport on `127.0.0.1:17366` was reachable and the local catalog reported 216 tools.
...
Result: succeeded. The verifier reported 216 tools, 3 resources, and 2 prompts. Path security rejected `C:\`, `%USERPROFILE%`, `%USERPROFILE%\.ssh`, and `%USERPROFILE%\AppData\Roaming`.
...
For this verification run, the host HTTP service was restarted after the latest build so Docker could query the rebuilt 216-tool server. A direct Streamable HTTP MCP probe reported `toolCount: 216`, `hasObjectiveReadiness: true`, `hasLaunchAudit: true`, `objectiveStatus: "ready_for_default_clients_pending_live_bridge"`, `auditMode: "ready_for_offline_planning"`, `okForDefaultClientUse: true`, `bridgeReachable: false`, and `safeToolCount: 140`. The objective report and launch audit include LiveAPI control coverage for write-gated supported track/scene/sample/MIDI/mixer/marker actions plus explicit unsupported boundaries for native device insertion, automation breakpoint writes, and quantization. The rebuilt HTTP surface also exposes the user-gated concept device UI session only outside the safe client allowlist; real UI/mouse work still requires `ABLETON_MCP_ENABLE_UI_CONTROL=1`.
...
Result: succeeded under WSL with 216 tools, 3 resources, and 2 prompts. The native WSL verifier ran the MCP catalog, security, runtime, bridge-mock, and sample-search checks. Platform path security rejected `/`, `%USERPROFILE%`, `%USERPROFILE%/.ssh`, and `%USERPROFILE%/AppData/Roaming`.
