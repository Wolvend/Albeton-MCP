...
Resources: 3
Prompts: 2
HyperNimbus Docker MCP enabled tools: 138
...
Result: succeeded. Doctor reported 8 checks, 0 failures, and 2 runtime warnings because the UI driver on `127.0.0.1:17365` and Max for Live bridge on `127.0.0.1:17364` were not loaded. The HTTP transport on `127.0.0.1:17366` was reachable and the local catalog reported 214 tools.
...
Result: succeeded. The verifier reported 214 tools, 3 resources, and 2 prompts. Path security rejected `C:\`, `%USERPROFILE%`, `%USERPROFILE%\.ssh`, and `%USERPROFILE%\AppData\Roaming`.
...
Result: succeeded. HyperNimbus still has `ableton-mcp` active as a remote MCP server. The verifier parsed Docker's enabled tool list and reported `expectedAllowedTools: 138`, `observedAllowedTools: 138`, `missingSafeTools: []`, `unexpectedAbletonTools: []`, and `unexpectedRiskyTools: []`.
...
Result: succeeded. The command exported a backup to `diagnostics/runtime/docker-mcp/hypernimbus.before.yaml`, replaced the local `ableton-mcp` catalog entry, disabled all Ableton MCP tools, and enabled the 138-tool safe allowlist. Docker reported an OAuth discovery 404 for the local endpoint, which is expected because this server does not expose OAuth metadata.
...
For this verification run, the host HTTP service was restarted after the latest build so Docker could query the rebuilt 214-tool server. A direct Streamable HTTP MCP probe reported `toolCount: 214`, `hasLaunchAudit: true`, `auditMode: "ready_for_offline_planning"`, `okForDefaultClientUse: true`, `bridgeReachable: false`, and `safeToolCount: 138`. The launch audit includes LiveAPI control coverage for write-gated supported track/scene/sample/MIDI/mixer/marker actions plus explicit unsupported boundaries for native device insertion, automation breakpoint writes, and quantization. The rebuilt HTTP surface also exposes the user-gated concept device UI session only outside the safe client allowlist; real UI/mouse work still requires `ABLETON_MCP_ENABLE_UI_CONTROL=1`.
...
Result: succeeded after restarting the local HTTP listener on `127.0.0.1:17366`. Docker loaded the HyperNimbus profile and listed `ableton-mcp` with 138 enabled safe tools.

Direct Streamable HTTP probe against `http://127.0.0.1:17366/mcp` reported `toolCount: 214`, `hasLaunchAudit: true`, `auditMode: "ready_for_offline_planning"`, `okForDefaultClientUse: true`, `bridgeReachable: false`, and `safeToolCount: 138`.
...
Result: succeeded under WSL with 214 tools, 3 resources, and 2 prompts. It ran 99 tests, lint, doctor, release check, safe sweep, all-tool sweep, MCP verifier, and npm audit. WSL doctor reported expected warnings for HTTP/UI/Max bridge listeners from the Linux namespace. Platform path security rejected `/`, `%USERPROFILE%`, `%USERPROFILE%/.ssh`, and `%USERPROFILE%/AppData/Roaming`.
