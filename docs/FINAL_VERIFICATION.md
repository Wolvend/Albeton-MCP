...
Resources: 3
Prompts: 2
HyperNimbus Docker MCP enabled tools: 123
...
Result: succeeded. The verifier reported 198 tools, 3 resources, and 2 prompts. Path security rejected `C:\`, `%USERPROFILE%`, `%USERPROFILE%\.ssh`, and `%USERPROFILE%\AppData\Roaming`.
...
Result: succeeded. HyperNimbus still has `ableton-mcp` active as a remote MCP server. The verifier parsed Docker's enabled tool list and reported `expectedAllowedTools: 123`, `observedAllowedTools: 123`, `missingSafeTools: []`, `unexpectedAbletonTools: []`, and `unexpectedRiskyTools: []`.
...
The host HTTP service was restarted after the latest build so Docker could query the rebuilt 198-tool server. A direct Streamable HTTP MCP probe reported `toolCount: 198` and `hasScorecard: true`.
...
Result: succeeded. Docker loaded the HyperNimbus profile and listed `ableton-mcp` with 123 tools.
...
Result: succeeded under WSL with 198 tools, 3 resources, and 2 prompts. Platform path security rejected `/`, `%USERPROFILE%`, `%USERPROFILE%/.ssh`, and `%USERPROFILE%/AppData/Roaming`.
