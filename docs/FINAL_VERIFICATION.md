...
Resources: 3
Prompts: 2
HyperNimbus Docker MCP enabled tools: 116
...
Result: succeeded. The verifier reported 191 tools, 3 resources, and 2 prompts. Path security rejected `C:\`, `%USERPROFILE%`, `%USERPROFILE%\.ssh`, and `%USERPROFILE%\AppData\Roaming`.
...
Result: succeeded. HyperNimbus still has `ableton-mcp` active as a remote MCP server with the 116-tool safe allowlist.
...
The host HTTP service was restarted after the latest build so Docker could query the rebuilt 191-tool server.
...
Result: succeeded. Docker loaded the HyperNimbus profile and listed `ableton-mcp` with 116 tools.
...
Result: succeeded under WSL with 191 tools, 3 resources, and 2 prompts. Platform path security rejected `/`, `%USERPROFILE%`, `%USERPROFILE%/.ssh`, and `%USERPROFILE%/AppData/Roaming`.
...
- HyperNimbus uses the safe tool allowlist. The bridge capability and concept mix-plan tools are read-only and included; color/write tools stay out of that allowlist and still require `dry_run=false` plus `ABLETON_MCP_ENABLE_WRITE=1` for real execution.
