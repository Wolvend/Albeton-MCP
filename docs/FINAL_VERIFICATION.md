...
Resources: 3
Prompts: 2
HyperNimbus Docker MCP enabled tools: 122
...
Result: succeeded. The verifier reported 197 tools, 3 resources, and 2 prompts. Path security rejected `C:\`, `%USERPROFILE%`, `%USERPROFILE%\.ssh`, and `%USERPROFILE%\AppData\Roaming`.
...
Result: succeeded. HyperNimbus still has `ableton-mcp` active as a remote MCP server. The verifier parsed Docker's enabled tool list and reported `expectedAllowedTools: 122`, `observedAllowedTools: 122`, `missingSafeTools: []`, `unexpectedAbletonTools: []`, and `unexpectedRiskyTools: []`.
...
The host HTTP service was restarted after the latest build so Docker could query the rebuilt 197-tool server.
...
Result: succeeded. Docker loaded the HyperNimbus profile and listed `ableton-mcp` with 122 tools.
...
Result: succeeded under WSL with 197 tools, 3 resources, and 2 prompts. Platform path security rejected `/`, `%USERPROFILE%`, `%USERPROFILE%/.ssh`, and `%USERPROFILE%/AppData/Roaming`.
