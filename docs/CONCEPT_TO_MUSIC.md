...
- `ableton_get_production_readiness`
  ...
  - Reports current gates, Docker/OpenClaw/Codex connection posture, bridge reachability, concept workflow readiness, safety posture, and exact next calls.
...
24. `ableton_render_concept_device_catalog_matches`
   - Read-only.
   - Matches staged device-chain names against the already-indexed Ableton catalog cache for `.adg`, `.adv`, `.amxd`, and optionally plugin-preset entries.
