...
The MCP tool `ableton_mcp_get_client_bootstrap_bundle` returns one machine-readable setup bundle with stdio/HTTP defaults, OpenClaw commands, Docker MCP profile scripts, safe include CSV, provider-host notes for OpenRouter/Gemini/llama.cpp/Antigravity, verification commands, and guardrails. `ableton_mcp_get_objective_readiness_report` returns the objective-level status for default client safety, Docker/OpenClaw profile readiness, concept-to-music readiness, live bridge reachability, write gates, optional UI/mouse control, and remaining runtime blockers. `ableton_mcp_get_launch_readiness_audit` returns a compact read-only checklist for safe defaults, client profile state, concept workflow readiness, bridge reachability, optional UI-driver gating, and LiveAPI control coverage. That coverage map separates write-gated supported actions such as track/scene creation, approved sample clip creation, MIDI note insertion, mixer/send changes, and markers from intentionally unsupported paths such as native device insertion and automation breakpoint writes. `ableton_mcp_get_client_connection_profiles` and `ableton_mcp_get_safe_tool_allowlist` remain available as narrower calls. After connecting, call `ableton_get_production_readiness` to see whether the environment is in offline-planning, live-read/dry-run, or write-ready mode, then call `ableton_plan_agent_music_session` to get the exact side-effect-free phase plan for turning a mood/place brief into concept, sample, arrangement, approval, and delivery calls. Use `ableton_curate_concept_samples` after a stored concept plan exists to map layers to licensed sample candidates before any staged download, use `ableton_render_concept_device_chain_spec` to review layer device order and parameter hints, use `ableton_render_concept_device_catalog_matches` to match those device names against the already-indexed Ableton catalog cache, use `ableton_plan_concept_device_ui_placement` to plan any user-gated foreground device placement without moving the mouse, and use `ableton_render_concept_execution_action_matrix` before approval to audit every generated bridge action and gate. `ableton_begin_concept_device_ui_session` is exposed only as an optional gated workflow in the bootstrap bundle; it is excluded from the Docker/OpenClaw safe include list and requires `ABLETON_MCP_ENABLE_UI_CONTROL=1`. For a local Codex/OpenClaw-style dry run of the whole safe workflow, run `.\launch.ps1 concept-demo -SkipSetup`.
