...
const renderSlug = safeRenderSlug(process.env.ABLETON_MCP_RENDER_SLUG || "there-was-never-a-door");
...
const SR = 44100;
const BPM = 56;
const BASE_DURATION = 168;
const DURATION = 253;
const TIME_SCALE = DURATION / BASE_DURATION;
const N = DURATION * SR;
const SECTION = 28;
...
function addSample(bus, src, opt) {
  const time = scoreTime(opt.time ?? 0);
  const srcSec = opt.src ?? 0;
  const rate = opt.rate ?? 1;
  const lengthSec = scoreLength(opt.length ?? 8);
...
function addTone(bus, opt) {
  const start = Math.floor(scoreTime(opt.time) * SR);
  const len = Math.floor(scoreLength(opt.length) * SR);
  const [pl, pr] = panGains(opt.pan ?? 0);
...
function addChangingRoomTail(bus, wet = 0.06) {
  ...
  for (const room of rooms) {
    const start = Math.floor(scoreTime(room.start) * SR);
    const end = Math.floor(scoreTime(room.end) * SR);
...
function applySubCollapse() {
  const dropStart = Math.floor(scoreTime(142) * SR);
  const dropEnd = Math.floor(scoreTime(153.5) * SR);
...
function writeWav(file, l, r) {
  ...
  let p = 44;
  for (let i = 0; i < N; i++) {
    writeInt24LE(b, Math.round(clamp(l[i], -1, 1) * 8388607), p); p += 3;
    writeInt24LE(b, Math.round(clamp(r[i], -1, 1) * 8388607), p); p += 3;
  }
...
function arrangeConcreteRoom() {
  addNoise(stems.concrete, { time: 0, length: BASE_DURATION, gain: 0.072, lowpass: 0.0009, band: 0.07, attack: 8, release: 9, crackleThreshold: 0.99995, crackle: 0.045 });
  for (const hum of [49.7, 59.8, 99.4, 119.6, 183.5]) {
    addTone(stems.concrete, { time: 0, length: BASE_DURATION, freq: hum, freqEnd: hum * (0.998 + rand() * 0.006), gain: hum < 70 ? 0.021 : 0.0085, attack: 8, release: 9, pan: rand() * 0.8 - 0.4 });
  }
...
function arrangeTapeArtifacts() {
  addNoise(stems.tape, { time: 0, length: BASE_DURATION, gain: 0.026, lowpass: 0.0035, band: 0.11, attack: 2, release: 2, crackleThreshold: 0.99908, crackle: 0.62 });
  for (let t = 2; t < BASE_DURATION; t += 1.85 + rand() * 2.4) {
    if (rand() < 0.38) {
...
fs.writeFileSync(attrOut, [
  ...
  "",
  "Direction: original 4:13 cinematic horror/liminal ambient composition using public-domain/PD-marked source recordings.",
  "This is a new render path and not a revision of the previous occult-liminal-backrooms masters.",
...
console.log(JSON.stringify({
  ok: true,
  title: "There Was Never a Door",
  bpmReference: BPM,
  durationSeconds: DURATION,
  bitDepth: 24,
  timeScaleFromPreviousSketch: Number(TIME_SCALE.toFixed(6)),
  masterWavOut,
  masterMp3Out,
  stagingMaster,
  attrOut,
