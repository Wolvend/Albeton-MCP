...
const BrowserCategory = z.enum(["all", "instruments", "audio_effects", "midi_effects", "max_for_live", "plugins", "drums", "sounds", "clips", "samples"]).default("all");
const BrowserPath = z.string()
  .min(1)
  .max(500)
  .regex(/^[A-Za-z0-9 _./:#-]+$/)
  .refine((value) => !value.split(/[\\/]+/).some((part) => part === "." || part === ".."), "Browser paths cannot contain traversal segments.");
...
toolDefs.push(
  ...
  { name: "ableton_get_browser_tree", description: "Read a bounded Ableton Browser tree from the LiveAPI bridge without loading devices or samples.", inputSchema: { category: BrowserCategory, max_items: z.number().int().min(1).max(64).default(16), max_depth: z.number().int().min(0).max(2).default(1) }, annotations: ro, handler: async (args) => bridgeRead("browser_tree", { category: args.category === "all" ? "" : args.category, max_items: args.max_items, max_depth: args.max_depth }) },
  { name: "ableton_get_browser_items_at_path", description: "Read children for a bounded Ableton Browser path or bridge path_hint without loading anything.", inputSchema: { path: BrowserPath, max_items: z.number().int().min(1).max(64).default(24), max_depth: z.number().int().min(0).max(2).default(1) }, annotations: ro, handler: async (args) => bridgeRead("browser_items_at_path", { path: args.path, max_items: args.max_items, max_depth: args.max_depth }) },
  ...
  { name: "ableton_get_arrangement_clips", description: "List Arrangement View clips for one track through the LiveAPI bridge when Live exposes arrangement_clips.", inputSchema: { track_index: TrackIndex.default(0), page: z.number().int().min(1).default(1), pageSize: z.number().int().min(1).max(64).default(32) }, annotations: ro, handler: async (args) => bridgeRead("arrangement_clips", { track_index: args.track_index, page: args.page, pageSize: args.pageSize }) },
  { name: "ableton_switch_to_arrangement_view", description: "Switch Ableton to Arrangement view through the gated bridge when supported; dry-run by default.", inputSchema: { ...DryRun }, annotations: rw, handler: async (args) => typedBridgeWrite("ableton_switch_to_arrangement_view", args, { target: "application_view", view: "Arranger" }) },
  { name: "ableton_set_arrangement_time", description: "Move the Arrangement playhead to a beat time through the gated bridge; dry-run by default.", inputSchema: { time: BeatTime, ...DryRun }, annotations: rw, handler: async (args) => typedBridgeWrite("ableton_set_arrangement_time", args, { target: "current_song_time", time: args.time }) },
  { name: "ableton_duplicate_session_clip_to_arrangement", description: "Copy a Session View clip to the Arrangement timeline when LiveAPI exposes duplicate_clip_to_arrangement.", inputSchema: { ...TrackClipRef, destination_time: BeatTime, ...DryRun }, annotations: rw, handler: async (args) => typedBridgeWrite("ableton_duplicate_session_clip_to_arrangement", args, { source: { track_index: args.track_index, clip_slot_index: args.clip_slot_index }, destination: { arrangement_time: args.destination_time } }) },
  { name: "ableton_load_drum_kit", description: "Plan loading a Drum Rack and kit from Browser paths; real Browser loading remains unsupported until the bridge proves a safe load_item path.", inputSchema: { track_index: TrackIndex, rack_uri: z.string().min(1).max(300), kit_path: BrowserPath, ...DryRun }, annotations: rw, handler: async (args) => unsupportedProfessionalWrite("ableton_load_drum_kit", args, { track_index: args.track_index, rack_uri: args.rack_uri, kit_path: args.kit_path, discovery: [{ name: "ableton_get_browser_tree", arguments: { category: "drums", max_items: 24, max_depth: 2 } }, { name: "ableton_get_browser_items_at_path", arguments: { path: args.kit_path, max_items: 24, max_depth: 1 } }] }, "Browser load_item/device insertion is not enabled in this bridge because it can change the selected track and device chain without a proven atomic rollback path.") },
