...
toolDefs.push(
  ...
  { name: "ableton_apply_groove", description: "Plan applying groove to a clip; bridge returns unsupported until a reliable LiveAPI groove path is available.", inputSchema: { ...TrackClipRef, groove: z.string().min(1).max(128).default("selected"), amount: z.number().min(0).max(1).default(0.5), ...DryRun }, annotations: rw, handler: async (args) => typedBridgeWrite("ableton_apply_groove", args, { target: "clip_groove", track_index: args.track_index, clip_slot_index: args.clip_slot_index, groove: args.groove, amount: args.amount }) },
